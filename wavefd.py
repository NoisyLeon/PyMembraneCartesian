...
* :func:`membrane`: Simulates SH membrane elastic waves using
...
This script was the wavefd in fatiando by Leonardo Uieda.

Modified by Lili Feng to simulate 2D membrane wave propagation.
...
class GaussSourceLF(MexHatSource):
    ...
    * delay : float
    ...
    def __init__(self, x, z, area, shape, amp, frequency, delay=None):
        super(GaussSourceLF, self).__init__(x, z, area, shape, amp,
                                          frequency, delay)
        if delay is None:
            self.delay = 3.0 / frequency
    ...
    def __call__(self, time):
        t = time - self.delay
        psi = self.amp*( t * numpy.exp( - (numpy.pi**2) * 2.* self.f2 * (t**2) ) )
        return psi
...
def _add_pad_membrane(array, pad, shape):
    """
    Pad the array with the values of the borders
    """
    array_pad = numpy.zeros(shape, dtype=numpy.float)
    array_pad[pad:-pad, pad:-pad] = array ### LF
    for k in xrange(pad):
        array_pad[pad:-pad, k] = array[:, 0]
        array_pad[pad:-pad, -(k + 1)] = array[:, -1]
    for k in xrange(pad):
        array_pad[-(pad - k), :] = array_pad[-(pad + 1), :]
        array_pad[k, :] = array_pad[pad, :]
    return array_pad

    
def membrane_sh(mu, density, area, dt, iterations, sources, stations=None,
               snapshot=None, padding=50, taper=0.005):
    """
    Simulate SH waves(membrane wave) using the Equivalent Staggered Grid (ESG) finite
    ...
    Uses absorbing boundary conditions (Gaussian taper) in the upper, lower, left and
    right boundaries. 
    Parameters:
    ...
    * t, uy, seismograms : int, 2D-array and list of 1D-arrays
    ...
    mu_pad = _add_pad_membrane(mu, pad, (nz, nx))
    dens_pad = _add_pad_membrane(density, pad, (nz, nx))
    ...
    for src in sources:
        i, j = src.indexes()
        u[1, i + pad, j + pad] += -(dt ** 2 / density[i, j]) * src(0) ### LF
    # Update seismograms
    for station, seismogram in zip(stations, seismograms):
        i, j = station
        seismogram[0] = u[1, i + pad, j + pad]
    if snapshot is not None:
        yield 0, u[1, pad:-pad, pad:-pad], seismograms
    for iteration in xrange(1, iterations):
        ...
        _apply_damping_membrane(u[t], nx, nz, pad, taper)
        # not PML yet or anything similar
        _nonreflexive_sh_boundary_conditions(u[tp1], u[t], nx, nz, dt, dx, dz,
                                             mu_pad, dens_pad)
        # Damp the regions in the padding to make waves go to infinity
        _apply_damping_membrane(u[tp1], nx, nz, pad, taper)
        for src in sources:
            i, j = src.indexes()
            u[tp1, i+pad, j +pad] += - (dt ** 2 / density[i, j]) * src(iteration * dt) ### LF
        # Update seismograms
        for station, seismogram in zip(stations, seismograms):
            i, j = station
            seismogram[iteration] = u[tp1, i+pad, j + pad]
        if snapshot is not None and iteration % snapshot == 0:
            yield iteration, u[tp1, pad:-pad, pad:-pad], seismograms
    yield iteration, u[tp1, pad:-pad, pad:-pad], seismograms


def membrane(vel, area, dt, iterations, sources, stations=None,
           snapshot=None, padding=50, taper=0.005):
    """

    Simulate membrane waves using an explicit finite differences scheme 4th order
    ...
    * i, u, seismograms : int, 2D-array and list of 1D-arrays
    ...
    nx, nz = numpy.shape(vel)  # get simulation dimensions
    x1, x2, z1, z2 = area
    dz, dx = (z2 - z1) / (nz -1 ), (x2 - x1) / (nx -1 )
    # print dz, dx
    if dz != dx:
    ...
    nz += 2 * pad ### LF
    # Pad the velocity as well
    vel_pad = _add_pad_membrane(vel, pad,  (nz, nx))
    # Pack the particle position u at 2 different times in one 3d array
    # u[0] = u(t-1)
    # u[1] = u(t)
    # The next time step overwrites the t-1 panel
    ...
    for src in sources:
        i, j = src.indexes()
        u[1, i + pad, j + pad] += -((vel[i, j] * dt) ** 2) * src(0)
    # Update seismograms
    for station, seismogram in zip(stations, seismograms):
        i, j = station
        seismogram[0] = u[1, i + pad, j + pad]
