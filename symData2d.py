...
class FatiandoDataBase(object):
    def __init__(self, enx, enz, xmax, zmax, ds, xmin=0, zmin=0, dx=1, dz=1):
    ...
    def ReadSeismograms(self, datadir, PRX='MEM2D_'):
        print 'Start Reading Seismograms!'
        for i in np.arange(self.Nx):
            for j in np.arange(self.Nz):
                X=i*self.dx;
                Z=j*self.dz;
                tr_id=str(int(X))+'_'+str(int(Z));
                sacfname=datadir +'/'+ PRX + tr_id +'.SAC';
                if not os.path.isfile(sacfname):
                    print 'SAC File: '+sacfname+' NOT exist!';
                    continue;
                tr=obspy.read(sacfname)[0];
                tr.stats.tr_id=PRX + tr_id +'.SAC';
    ...
    def aftan(self, outdir, fhlen=0.008, inftan=InputFtanParam() ):
        ...
        for mytrace in self.symStream:
            if mytrace.stats.sac.dist < 0.1:
                print 'Too close to source, skip aftan for: '+mytrace.stats.tr_id;
                continue;
            Strace=symtrace(mytrace.data, mytrace.stats);
            fPRX=outdir+'/'+Strace.stats.tr_id;
            print fPRX
    ...
    def GetField2dFile(self, datadir, outdir, perLst, datatype='both', outfmt='txt', PRX='MEM2D_'):
        if not os.path.isdir(outdir):
            os.makedirs(outdir);
        for per in perLst:
            TphLst=np.array([]);
            ...
            for i in np.arange(self.Nx):
                for j in np.arange(self.Nz):
                    X=i*self.dx;
                    Z=j*self.dz;
                    if X==self.xmax/2 or Z==self.zmax/2:
                        continue;
                    tr_id=str(int(X))+'_'+str(int(Z));
                    sacfname=datadir +'/'+ PRX + tr_id +'.SAC';
...
def FDataBaseaftan(STrace, outdir, fhlen=0.008, inftan=InputFtanParam() ):
    if STrace.stats.sac.dist < 0.1:
        print 'Too close to source, skip aftan for: '+STrace.stats.tr_id;
        return;
